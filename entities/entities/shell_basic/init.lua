...
function ENT:Initialize()
	...
	local trail = util.SpriteTrail(self, 0, Color(255,0,0), true, 18, 1, 1, 1/(15+1)*0.5, "trails/plasma.vmt")
end
	...
	for _,Ent in pairs (ents.FindInSphere( self:GetPos() , 200 ) ) do
		if Ent:GetClass() == "prop_physics" and Ent:GetModel() == "models/props_junk/watermelon01.mdl" then
			...
			if Ply.Shield > 0 then
				Ply.Shield = Ply.Shield - Distance / 4
			else
				Ply.HP = Ply.HP - Distance / 4
				if Ply.HP <= 0 then
...
function ENT:Think()
	...
	if CurTime() - self.Created > 0.4 then
		for _,Ent in pairs (ents.FindInSphere( self:GetPos() , 60 , "tankhover" ) ) do
			if Ent:GetClass() == "tankhover" or Ent:GetClass() == "mine" then
				self:Remove()
			end
		end
	end
